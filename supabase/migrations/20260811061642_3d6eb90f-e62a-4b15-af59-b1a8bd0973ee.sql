CREATE OR REPLACE FUNCTION private.org_usage_summary(p_org_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION private.org_usage_summary(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION private.org_usage_summary(uuid) TO authenticated, service_role;

-- Public wrapper runs as the caller (SECURITY INVOKER), so it is not an RLS bypass surface.
CREATE OR REPLACE FUNCTION public.get_org_usage_summary(p_org_id uuid)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT private.org_usage_summary(p_org_id);
$function$;
