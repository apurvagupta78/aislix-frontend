...
import { MpBadge, mpStageTone } from "@/components/design-system/MpBadge";
    ...
    <section className="overflow-hidden rounded-xl border border-line bg-white shadow-card">
      <div className="border-b border-line bg-white px-4 py-3 sm:px-5">
        ...
        <div className="relative mt-3 max-w-sm">
          <Search className="absolute left-2.5 top-2.5 size-4 text-mp-muted" />
          <Input
            className="h-9 border-line bg-canvas pl-8 text-xs"
      ...
      {filtered.length === 0 ? (
        <p className="px-5 py-10 text-center text-sm text-mp-muted">
        ...
        <>
          <Table>
            ...
            <TableBody>
              {slice.map((row) => (
                <TableRow key={row.auditId} className="cursor-pointer" onClick={() => openRow(row)}>
                  <TableCell className="whitespace-nowrap text-mp-muted">{formatDate(row.date)}</TableCell>
                  <TableCell className="font-mono text-xs font-medium">{formatAssignmentId(row.auditId)}</TableCell>
                  <TableCell className="font-medium text-navy">{row.location}</TableCell>
                  <TableCell className="text-mp-muted">{row.city}</TableCell>
                  <TableCell>{row.template}</TableCell>
                  <TableCell>{row.assignedTo}</TableCell>
                  <TableCell>
                    <MpBadge tone={mpStageTone(row.stage)}>{row.stage}</MpBadge>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          <div className="flex items-center justify-between border-t border-line bg-canvas px-4 py-2.5 text-xs text-mp-muted">
            <span>
