...
export async function fetchWorkspaceDashboard(
  ...
  since.setDate(since.getDate() - dateRangeToDays(filters.dateRange));
  const sinceIso = since.toISOString();

  const [storesRes, scansRes, subscriptionRes, actionsRes] = await Promise.all([
    supabase.from("stores").select("id, name").eq("org_id", orgId).eq("status", "active").order("name"),
    (() => {
      let q = supabase
        .from("shelf_scans")
        .select(
          "id, created_at, store_id, category, osa_percent, planogram_compliance_percent, shelf_health_score, share_of_shelf_percent, total_products, photo_count, stores(name)",
        )
        .eq("org_id", orgId)
        .eq("status", "completed")
        .gte("created_at", sinceIso)
        .order("created_at", { ascending: true });
      if (filters.storeId !== "all") q = q.eq("store_id", filters.storeId);
  ...
  let scans = (scansRes.data ?? []) as ScanRow[];
  const categories = [...new Set(scans.map((s) => s.category).filter(Boolean) as string[])].sort();
  ...
  if (filters.role !== "all") {
    scans = scans.filter((s) => scanRole(metricsMap.get(s.id) ?? null) === filters.role);
  }
  ...
  return {
    ...
    role_visual,
    filter_options: { stores: allStores, categories },
  };
}
